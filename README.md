...
### iii
-Balanced datasets are extremeley important. Models learn what they are exposed to. So if they are exposed to unbalanced datasets then they will learn to representations of over represented classes. So only groups that are more present such as majority race/ethnic groups, middle age patients etc would be on which the model performance would be accurate. In unbalanced cases, the ability of the models to diagnose, prognosticate, classfy and stratify would be hampered and some population may get over or under diagnosed. Balanced dataset give a comprehensive representation of population leading to accurate insights and generalizability
-Implications could include model only trains to recognize one population group over the other. So while its performance to identify one group might be high it isnt generalizable. Similarly for other associated clinical endpoints being looked at, model might only be able to perform well for othat one group which was prevalent in the training set. This could lead to false positive and false negatives for under represented classes leading to under,over or miss diagnosis. In our case while AUC was sort of stable, F1 score shifted towards majority class (here sex). Challenge is that model should be able to phenotype patients and capture underlying patterns and population specific susceptabilities. Also it always is a challenge to create a balanced dataset as well as to ensure fair performance. 
- Strategies such as class weighted training of models, over sampling, under sampling of classes based on theur prevalence. Bootstrapped performace metrics could give better picture of model performance. Deep subgroup analysis could help too. Data augmentation and well as synthetic data generation are useful in such cases too
